...
//                "expire", expire));
    ...
    public DashboardResponse testSerialization(String token) {
        String key = "dashboardData::" + token;
        DashboardResponse response = (DashboardResponse) redisTemplate.opsForValue().get(key);
        System.out.println("Retrieved from cache: " + response);
        return response;
    }

    @GetMapping("/test")
    public ResponseEntity<DashboardResponse> test(@RequestHeader("Authorization") String authHeader) {
        return ResponseEntity.ok(testSerialization(authHeader));
    }
}